...
- 対象求職者（当初）: 惣流アスカ（求職者ID 18）のみ。`CAREER#CAREER_ID EQ 18` で絞る
...
| `runCareerAction()` | 15分 | 要件4。**`enabled: false`**（Phase6 まで関数が存在しない） |
...
| Python 版 | GAS 版 |
|---|---|
| `--check` | `checkSetup()` / `checkCareerStatus()` / `checkProgressFlow()` |
| `--bootstrap` | `bootstrapCareerStatus()` / `bootstrapProgressFlow()` |
| `--once --watcher X` | `runCareerStatus()` / `runProgressFlow()` |
| `--dry-run` | `runCareerStatusDryRun()` / `runProgressFlowDryRun()` |
...
| トリガーの登録 | ✅ `createTriggers()` で3本。要件4（`enabled: false`）は作られない |
